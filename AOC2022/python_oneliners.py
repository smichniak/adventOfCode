...
print('Day 4:')
print(sum([z >= x and w <= y or x >= z and y <= w for x, y, z, w in map(lambda x: map(int, x.replace('-', ',').split(',')), open('input/day4.in', 'r'))]))
print(sum([x <= z <= y or x <= w <= y or z <= x <= w or z <= y <= w for x, y, z, w in map(lambda x: map(int, x.replace('-', ',').split(',')), open('input/day4.in', 'r'))]))
print()
